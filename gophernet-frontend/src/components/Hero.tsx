    ...
    <div className="w-full max-w-lg mx-auto lg:mx-0">
      <div className="bg-slate-900 rounded-lg shadow-2xl border border-slate-700 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 py-2.5 bg-slate-800 border-b border-slate-700">
          ...
          <button
            onClick={handleCopy}
            className="text-slate-400 hover:text-white transition-colors text-xs font-medium flex items-center gap-1.5 px-2 py-1 rounded hover:bg-slate-700"
            title="Copy commands"
          >
        ...
        <div className="px-6 py-5 font-mono text-sm text-slate-200 space-y-2">
          <p><span className="text-green-400">$</span> git clone https://github.com/yourname/gophernet</p>
          <p><span className="text-green-400">$</span> cd gophernet-backend</p>
          <p><span className="text-green-400">$</span> go run main.go</p>
          <p><span className="text-green-400">$</span> curl -X POST http://localhost:3000/publish</p>
          <p className="pt-2 text-green-400">
            ✓ GopherNet Broker running on :3000
          </p>
        </div>
